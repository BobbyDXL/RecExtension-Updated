...
self.addEventListener('unhandledrejection', (event) => {
    console.error('[RecFix] Unhandled promise rejection:', {
        reason: event.reason,
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        case 'checkAPIKey':
            ...
            console.log('[RecFix] API key status check: present');
            sendResponse({ hasKey: true });
            break;

        case 'getRemainingLimit':
            getRemainingLimit()
                .then(remaining => sendResponse({ remaining }))
                .catch(error => sendResponse({ error: error.message }));
            return true;

        case 'apiKeyUpdated':
            // Handle API key update notification
            console.log('[RecFix] Received API key update notification');
        ...
        case 'processManualUrls':
            try {
                validateAPIKey(); // Check API key before proceeding
                console.time('[RecFix] Processing manual URLs');
                handleManualUrls(request.urls)
                    .then(response => {
                        console.timeEnd('[RecFix] Processing manual URLs');
                        console.log('[RecFix] Manual URL processing complete:', {
                            urlsProcessed: request.urls.length,
                            recommendationsFound: response.recommendations?.length
                        });
                        sendResponse(response);
                    })
                    .catch(error => {
                        console.timeEnd('[RecFix] Processing manual URLs');
                        console.error('[RecFix] Error processing manual URLs:', {
                            error: error.message,
                            code: error.code,
                            stack: error.stack
                        });
                ...
                console.error('[RecFix] Validation error:', error);
                sendResponse({ error: error.message });
            }
            break;
        ...
        case 'fixFeed':
            ...
            checkRateLimit()
                .then(({ remaining }) => {
                    ...
                    return generateImprovedRecommendations(request.selectedVideos)
                        .then(response => {
                            console.timeEnd('[RecFix] Fixing feed');
                            console.log('[RecFix] Feed fix complete:', {
                                videosProcessed: request.selectedVideos.length,
        ...
        case 'saveRecommendations':
            ...
            saveRecommendationsToPlaylist(request.recommendations)
                .then(response => {
                    console.timeEnd('[RecFix] Saving recommendations');
                    console.log('[RecFix] Recommendations saved:', {
                        count: request.recommendations.length,
        ...
        case 'handleOAuthCode':
            handleOAuthCallback(request.code)
                .then(sendResponse)
                .catch(error => sendResponse({ error: error.message }));
            break;
...
async function getAuthToken() {
    console.log('[RecFix] Getting auth token');
    try {
        // First, try to get the token without interaction to check if we have a cached token
        try {
            const existingToken = await chrome.identity.getAuthToken({ interactive: false });
            if (existingToken) {
                // Revoke the token to force a new account selection
                const revokeUrl = `https://accounts.google.com/o/oauth2/revoke?token=${existingToken}`;
                await fetch(revokeUrl);
                await chrome.identity.removeCachedAuthToken({ token: existingToken });
                console.log('[RecFix] Removed and revoked previous auth token');
            }
        } catch (error) {
            console.log('[RecFix] No previous token to remove:', error.message);
        }

        // Get a new token with interactive login
        const result = await new Promise((resolve, reject) => {
            chrome.identity.getAuthToken({ 
                interactive: true
            }, (token) => {
                if (chrome.runtime.lastError) {
                    console.error('[RecFix] Chrome runtime error:', chrome.runtime.lastError);
        ...
        console.log('[RecFix] Auth token obtained successfully:', {
            tokenPresent: !!result,
            tokenLength: result?.length,
            tokenStart: result?.substring(0, 5) + '...'
        });

        return `Bearer ${result}`;
    } catch (error) {
        console.error('[RecFix] Error getting auth token:', {
            error: {
                message: error.message,
                name: error.name,
                stack: error.stack
    ...
    try {
        ...
        try {
            ...
            if (existingToken) {
                ...
                await fetch(revokeUrl);
                await chrome.identity.removeCachedAuthToken({ token: existingToken });
                console.log('[RecFix] Removed and revoked existing auth token during initialization');
            }
        } catch (error) {
            console.log('[RecFix] No existing token to remove during initialization:', error.message);
        }
    ...
    try {
        ...
        try {
            ...
            if (existingToken) {
                ...
                await fetch(revokeUrl);
                await chrome.identity.removeCachedAuthToken({ token: existingToken });
                console.log('[RecFix] Removed and revoked existing auth token during callback');
            }
        } catch (error) {
            console.log('[RecFix] No existing token to remove during callback:', error.message);
        }
